...
print(">>> Just log", file=sys.stdout, flush=True)

@app.on_event("startup")
async def startup():
    print(">>> Startup", file=sys.stdout, flush=True)
    if REDIS_URL:
        redis = aioredis.from_url(REDIS_URL, encoding="utf8", decode_responses=True)
        FastAPICache.init(RedisBackend(redis), prefix="api:cache")
        print(">>> FastAPICache backend:", FastAPICache.get_backend(), file=sys.stdout, flush=True)
...
async def list_users(collection_name: str):
    ...
    """
    print(">>> Cache backend in handler:", FastAPICache.get_backend(), file=sys.stdout, flush=True)
